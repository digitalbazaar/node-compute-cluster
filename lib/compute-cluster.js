...
ComputeCluster.prototype._onWorkerExit = function(pid) {
  ...
  return function (code) {
    ...
    var killedWorker = self._kids[pid];

    // if a job was running on this worker, notify that it won't finish
    if (killedWorker.job && killedWorker.job.cb) {
      killedWorker.job.cb("Worker killed while processing a job");
    }
